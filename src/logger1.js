...
var logger = {
  ...
  reportByIMG(paramStr) {
    let reportUrl = `${logger.baseUrl}/api/report/uploadShort`;
  ...
  getMetaData(defaultInfo) {
    ...
    if (logger.options.useCustomField) {
      let customFieldInfo = customFieldUtil.getCustomField(logger.options.customField);
...
(function () {
  ...
  let script = document.getElementById('rebugger');
  if (script) {
    apikey = script.getAttribute('apikey');
    let useCustomField = script.getAttribute('useCustomField');
    let silentAttr = script.getAttribute('silent');
    if (useCustomField && useCustomField == 'true') {
      useCustomField = true;
    }
  ...
  logger.options.today = `${new Date().getFullYear()}-${
    new Date().getMonth() + 1
  }-${new Date().getDate()}`;
  // 获取默认baseUrl
